...
extension WebSocket {
	...
	internal final class Handler:ChannelDuplexHandler {
		...
		internal init(url:URL, configuration:Client.Configuration, writeOp:WebSocketOpcode = .text) {
			self.url = url
			...
			switch self.frameParsingMode {
				...
				case .existingFrameFragments(var existingFrame):
					...
					guard existingFrame.type.opcode() == frame.opcode else {
						...
						switch frame.fin {
							case false:
								context.fireErrorCaught(Error.WebSocket.rfc6455Violation(.fragmentControlViolation(.steamOpcodeMismatch(existingFrame.type.opcode(), frame.opcode))))
							case true:
				...
				case .idle:
					switch frame.opcode {
						...
						default:
							#if DEBUG
							self.logger.error("received frame with opcode \(frame.opcode) but expected text or binary.")
							#endif
							context.fireErrorCaught(Error.WebSocket.rfc6455Violation(.fragmentControlViolation(.initiationWithNonContinuation(frame.opcode))))
							return
					}
