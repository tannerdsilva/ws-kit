...
extension URL {
	public struct Parser:Sendable {
		/// errors associated with parsing
		public enum Error:Swift.Error, Sendable {
			/// there was a buffer overflow
			case overflow
			/// unexpected content in buffer
			case unexpected
			/// buffer was empty
			case emptyString
			/// buffer was not valid UTF8
			case invalidUTF8
		}
		...
		/// - returns: Parser object if successful, or nil if the data was not valid UTF8
		...
		/// create a Parser object with a given string.
...
extension URL.Parser {
	...
	public init(_ parser:Self, range: Range<Int>) {
	...
	public func subParser(_ range: Range<Int>) -> Self {
...
extension URL.Parser {
	...
	public mutating func character() throws -> Unicode.Scalar {
	...
	public mutating func read(_ char: Unicode.Scalar) throws -> Bool {
	...
	public mutating func read(_ characterSet: Set<Unicode.Scalar>) throws -> Bool {
		let initialIndex = self.index
		let c = try character()
	...
	public mutating func read(_ string: String) throws -> Bool {
		let initialIndex = self.index
	...
	public mutating func read(count: Int) throws -> URL.Parser {
		var count = count
	...
	@discardableResult public mutating func read(until: Unicode.Scalar, throwOnOverflow: Bool = true) throws -> URL.Parser {
	...
	/// - Throws: .overflow
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(until characterSet: Set<Unicode.Scalar>, throwOnOverflow: Bool = true) throws -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd() {
	...
	/// - Throws: .overflow
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(until: (Unicode.Scalar) -> Bool, throwOnOverflow: Bool = true) throws -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd() {
	...
	/// - Throws: .overflow
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(until keyPath: KeyPath<Unicode.Scalar, Bool>, throwOnOverflow: Bool = true) throws -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd() {
	...
	/// - Throws: .overflow, .emptyString
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(untilString: String, throwOnOverflow: Bool = true, skipToEnd: Bool = false) throws -> URL.Parser {
	...
	/// - Returns: String read from buffer
	@discardableResult public mutating func readUntilTheEnd() -> URL.Parser {
		let startIndex = self.index
	...
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(while: Unicode.Scalar) -> Int {
		var count = 0
		while !self.reachedEnd(),
			unsafeCurrent() == `while`
		{
	...
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(while characterSet: Set<Unicode.Scalar>) -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd(),
	...
	/// - Parameter while: character set to check
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(while: (Unicode.Scalar) -> Bool) -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd(),
			`while`(unsafeCurrent())
		{
	...
	/// - Parameter while: character set to check
	/// - Returns: String read from buffer
	@discardableResult public mutating func read(while keyPath: KeyPath<Unicode.Scalar, Bool>) -> URL.Parser {
		let startIndex = self.index
		while !self.reachedEnd(),
	...
	public mutating func split(separator: Unicode.Scalar) -> [URL.Parser] {
...
extension URL.Parser {
	...
	public func current() -> Unicode.Scalar {
	...
	/// - Throws: .overflow
	public mutating func advance(by amount: Int) throws {
		var amount = amount
		while amount > 0 {
			guard !self.reachedEnd() else { throw Error.overflow }
	...
	/// - Throws: .overflow
	public mutating func retreat() throws {
	...
	/// - Throws: .overflow
	public mutating func retreat(by amount: Int) throws {
		var amount = amount
		while amount > 0 {
			guard self.index > self.range.startIndex else { throw Error.overflow }
			self.index = backOneUTF8Character(at: self.index)
	...
	public mutating func unsafeAdvance() {
		self.index = skipUTF8Character(at: self.index)
	}

	public mutating func unsafeAdvance(by amount: Int) {
		var amount = amount
		while amount > 0 {
			self.index = skipUTF8Character(at: self.index)
			amount -= 1
...
extension URL.Parser:Sequence {
	...
	public struct Iterator: IteratorProtocol {
		...
		private var parser: URL.Parser

		fileprivate init(_ parser: URL.Parser) {
			self.parser = parser
		}
...
// internal functions
extension URL.Parser {
	public func unsafeCurrent() -> Unicode.Scalar {
