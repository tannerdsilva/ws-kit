...
int8_t _cwskit_al_remove_try(_cwskit_atomiclist_keyed_aptr_t*_Nonnull base, const uint64_t key, _cwskit_ptr_t*_Nonnull ptr_out) {
	_cwskit_atomiclist_keyed_ptr_t current = atomic_load_explicit(base, memory_order_acquire);
	while (current != NULL) {
