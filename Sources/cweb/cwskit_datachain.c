...
_cwskit_ptr_t _cwskit_dc_consume(const _cwskit_datachainpair_ptr_t chain) {
	_cwskit_ptr_t consumed = NULL;
	if (__builtin_expect(_cwskit_dc_consume_next(atomic_load_explicit(&chain->base, memory_order_acquire), chain, &consumed), true)) {
		return (_cwskit_optr_t)consumed;
	} else {
		return (_cwskit_optr_t)NULL;
	}
}
